...
\section{Conclusion}
We studied an alternative framework for bandit learning. We explored how the type of random graph affected the algorithm's results. DuplExp3 does a good job estimating the revelation probability, even when graphs are not from the Erdos-Renyi distribution. We empirically showed that average independence number for Barabási-Albert graphs was higher than those of Erdos-Renyi graphs. This difference may explain the difference of performance for DuplExp3 on graphs that share the same edge proportion, but with different structure. It is hard in our setup to really exploit the distinctive degree distribution of Barabási-Albert graphs, since a new graph is created by the environment at each iteration. In a setup closer to the reality of social networks, it might be interesting to study regret bounds when the graph is known, or follows the Barabási-Albert evolution at each iteration. 
